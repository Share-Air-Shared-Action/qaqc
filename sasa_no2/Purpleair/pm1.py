...
df = pd.read_csv('purpleairprimary.csv', usecols = ['created_at','pm1_cf_atm_ugm3'])
df['created_at'] = pd.to_datetime(df['created_at'])

x = df['created_at']
y = df['pm1_cf_atm_ugm3']
m = round(df['pm1_cf_atm_ugm3'].mean(),3)
numzero =round(((df['pm1_cf_atm_ugm3'] == 0.000).sum()/(len(df['pm1_cf_atm_ugm3']))),5)*100
print(len(df['pm1_cf_atm_ugm3']))
n = df['pm1_cf_atm_ugm3'].count()
print (n)
st = round(df['pm1_cf_atm_ugm3'].std(),3)
font = {'family': 'serif',
        'color':  'black',
        'weight': 'normal',
        'size': 16,
...
no2_plot = df['pm1_cf_atm_ugm3'].hist(bins=50)
df['pm1_cf_atm_ugm3'] = df['pm1_cf_atm_ugm3'].replace(0, np.NaN)
print(df['pm1_cf_atm_ugm3'].mean())
mean = df['pm1_cf_atm_ugm3'].mean()
std = df['pm1_cf_atm_ugm3'].std()
nz = df['pm1_cf_atm_ugm3'].count()
print(nz)
outliers=round(((df['pm1_cf_atm_ugm3']>(mean + 4*std)).sum())/nz,5)*100
print(outliers)
...
no2_plot.set_title("PM1 ug/m3", fontsize = 36)
